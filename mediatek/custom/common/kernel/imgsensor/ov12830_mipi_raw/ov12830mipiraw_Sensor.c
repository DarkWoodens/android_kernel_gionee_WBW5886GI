...
//#define DEBUG_TEMP_INFO	
#define OTP_CALIBRATION

#ifdef DEBUG_TEMP_INFO	
...
#define Sleep(ms) mmDELAY(ms)

#ifdef OTP_CALIBRATION
#define RG_Ratio_Typical 0x17d
...
static int check_otp_wb(int index)
{
	...
	flag = flag >>6;
	// disable otp read
	OV12830_write_cmos_sensor(0x3d81, 0x00);
	// clear otp buffer
	for (i=0;i<16;i++) {
		OV12830_write_cmos_sensor(0x3d00 + i, 0x00);
	}
	OV12830DB("###check_otp_wb flag 0x%x###\n", flag);
	return flag;
}
...
static int check_otp_lenc(int index)
{
	...
	OV12830DB("###check_otp_lenc flag 0x%x###\n", flag);
	return flag;
}
...
static int update_otp_wb()
{
	...
	if (i>3) {
		...
		OV12830DB("###no valid wb OTP data!###\n");
		return 1;
	}
	...
	OV12830DB("###OTP DATA : rg 0x%x, bg 0x%x###\n", rg, bg);
...
static int update_otp_lenc()
{
	...
	if (i>3) {
		...
		OV12830DB("###no valid lenc OTP data!###\n");
		return 1;
	}
...
static void OV12830_Sensor_Init(void)
{
	...
	#ifdef OTP_CALIBRATION
	if (update_otp_wb()) {
		printk("!!!!!!update_otp_wb failed!\n");
	}
	if (update_otp_lenc()) {
		printk("!!!!!!update_otp_lenc failed!\n");
	}
	#endif	
}   /*  OV12830_Sensor_Init  */
...
#ifdef DEBUG_TEMP_INFO	
static void temp_work_fn(struct work_struct *work) 
{
	kal_uint16 temp;

	OV12830_write_cmos_sensor(0x4d0b, 0x80);
    temp = OV12830_read_cmos_sensor(0x4d0b); 
	printk("###############0x4d0b = 0x%x \n", temp);
}

void temp_timer_fn(unsigned long arg) {
	schedule_work(&temp_work);
	mod_timer(&temp_timer, jiffies+2*HZ);
...
UINT32 OV12830_MIPI_RAW_SensorInit(PSENSOR_FUNCTION_STRUCT *pfFunc)
{
	...
	#ifdef DEBUG_TEMP_INFO	
	INIT_WORK(&temp_work, temp_work_fn);
	init_timer(&temp_timer);
	temp_timer.function = temp_timer_fn;
	#endif
